    ...
    >
      ...
      {mounted && isHovering && (
        <>
          {/* Main cursor glow */}
          <div
            className="fixed pointer-events-none z-50 w-8 h-8 rounded-full bg-gradient-to-r from-blue-500/30 to-purple-500/30 blur-sm transition-all duration-150 ease-out"
            style={{
          ...
          {cursorTrail.map((point, index) => (
            <div
              key={point.id}
              className="fixed pointer-events-none z-40 w-2 h-2 rounded-full bg-gradient-to-r from-blue-400 to-purple-400 opacity-70"
              style={{
                left: point.x - 4,
                top: point.y - 4,
                transform: `scale(${(index + 1) / cursorTrail.length})`,
                opacity: ((index + 1) / cursorTrail.length) * 0.7,
                transition: "all 0.3s ease-out",
              }}
            />
          ))}

          {/* Floating sparkles around cursor */}
          <div
          ...
          >
            ...
            />
            <Sparkles
              className="absolute text-blue-400 animate-pulse opacity-50"
              size={8}
              style={{
            ...
            />
            <Sparkles
              className="absolute text-purple-400 animate-pulse opacity-40"
              size={10}
              style={{
